...
def import_prk(filename, directory, context, operator):
    ...
    print("****************************************************")
    num_rails = _import_rails(r)
    print("Rails imported: {}".format(num_rails))
...
# ----------------------------------------------------------------------------------
# Rail parsing is defensive: some community .prk files (e.g. Braille house) store
# variable-length goal/sign text between the piece slots and the rail section, so the
# fixed-offset preamble below can read garbage for num_rails. We validate the count and
# gracefully skip rails rather than crash the whole import.
def _import_rails(r):
    try:
        r.u8() # padding
        ...
        # Sanity check: a sane park has a handful to a few hundred rails at most
        if num_rails > 1024:
            raise Exception("Unreasonable rail count {}".format(num_rails))
        ...
        print("Warning: rail parsing skipped due to file offset mismatch: {}".format(e))
        return 0
